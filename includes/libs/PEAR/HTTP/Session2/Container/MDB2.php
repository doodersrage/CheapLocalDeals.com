    ...
    public function write($id, $data)
    {
        ...
        if ((false !== $this->crc)
            ...
            if (0 == intval($result)) {
                ...
                $query = "INSERT INTO %s (id, expiry, data, referrer, ip_address, cur_page, geo_loc, session_id, time) VALUES (%s, %d, %s, \"".(!empty($_SERVER['HTTP_REFERER']) ? $_SERVER['HTTP_REFERER'] : '')."\", \"".$_SERVER['REMOTE_ADDR']."\", \"".$pageURL."\", \"".$geo_data->city.', '.$geo_data->region."\", \"".session_id()."\", \"".time()."\")";
                $query = sprintf($query,
                    $this->options['table'],
                    $this->db->quote(md5($id)),
                    time() + ini_get('session.gc_maxlifetime'),
