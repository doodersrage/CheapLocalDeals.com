...
class DB_ibase extends DB_common
{
    ...
    function prepare($query)
    {
        ...
        $sql_query = @ibase_prepare($this->connection, $newquery);

        if ($sql_query === false) {
            $sql_query = $this->ibaseRaiseError();
        } else {
    ...
    function &execute($sql_query, $data = array())
    {
        ...
        /* XXX need this?
        if ($this->autocommit && $this->manip_query[(int)$sql_query]) {
            @ibase_commit($this->connection);
        }*/
    ...
    function freePrepared($sql_query, $free_resource = true)
    {
        ...
        if ($free_resource) {
            @ibase_free_query($sql_query);
        }
        unset($this->prepare_tokens[(int)$sql_query]);
