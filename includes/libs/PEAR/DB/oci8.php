...
class DB_oci8 extends DB_common
{
     ...
     *
     * @param resource $sql_query           the prepared statement's resource
     ...
     * @see DB_oci8::prepare()
     */
    function freePrepared($sql_query, $free_resource = true)
    {
        if (!is_resource($sql_query)) {
            return false;
        }
        if ($free_resource) {
            @ocifreestatement($sql_query);
        }
        if (isset($this->prepare_types[(int)$sql_query])) {
            unset($this->prepare_types[(int)$sql_query]);
            unset($this->manip_query[(int)$sql_query]);
        } else {
            return false;
        }
        return true;
    ...
    function prepare($query)
    {
        ...
        if (!$sql_query = @OCIParse($this->connection, $newquery)) {
            return $this->oci8RaiseError();
        }
        $this->prepare_types[(int)$sql_query] = $types;
        $this->manip_query[(int)$sql_query] = DB::isManip($query);
        $this->_prepared_queries[(int)$sql_query] = $newquery;
        return $sql_query;
    }
     ...
     * This option was added in Release 1.7.0.
     *
     * @param resource  $sql_query  a DB statement resource returned from prepare()
     ...
     * @see DB_oci8::prepare()
     */
    function &execute($sql_query, $data = array())
    {
        $data = (array)$data;
        $this->last_parameters = $data;
        $this->last_query = $this->_prepared_queries[(int)$sql_query];
        $this->_data = $data;

        $types = $this->prepare_types[(int)$sql_query];
        ...
        foreach ($data as $key => $value) {
            ...
            if (!@OCIBindByName($sql_query, ':bind' . $i, $data[$key], -1)) {
                $tmp = $this->oci8RaiseError($sql_query);
                return $tmp;
            }
        ...
        if ($this->autocommit) {
            $success = @OCIExecute($sql_query, OCI_COMMIT_ON_SUCCESS);
        } else {
            $success = @OCIExecute($sql_query, OCI_DEFAULT);
        }
        if (!$success) {
            $tmp = $this->oci8RaiseError($sql_query);
            return $tmp;
        }
        $this->last_stmt = $sql_query;
        if ($this->manip_query[(int)$sql_query] || $this->_next_query_manip) {
            $this->_last_query_manip = true;
            $this->_next_query_manip = false;
            $tmp = DB_OK;
        } else {
            $this->_last_query_manip = false;
            @ocisetprefetch($sql_query, $this->options['result_buffering']);
            $tmp = new DB_result($this, $sql_query);
        }
        return $tmp;
    }
    ...
    function tableInfo($result, $mode = null)
    {
        ...
        if (is_string($result)) {
            ...
            if (!$sql_query = @OCIParse($this->connection, $q_fields)) {
                return $this->oci8RaiseError(DB_ERROR_NEED_MORE_DATA);
            }
            if (!@OCIExecute($sql_query, OCI_DEFAULT)) {
                return $this->oci8RaiseError($sql_query);
            }
            
            $i = 0;
            while (@OCIFetch($sql_query)) {
                $res[$i] = array(
                    'table' => $case_func($result),
                    'name'  => $case_func(@OCIResult($sql_query, 1)),
                    'type'  => @OCIResult($sql_query, 2),
                    'len'   => @OCIResult($sql_query, 3),
                    'flags' => (@OCIResult($sql_query, 4) == 'N') ? 'not_null' : '',
                );
