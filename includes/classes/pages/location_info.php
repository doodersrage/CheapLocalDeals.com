...
class location_info_pg {
	...
	public function __construct() {
		global $cats_tbl, $adv_info_tbl, $adv_alt_loc_tbl, $api_load;
			
		// set location values
		$this->loc_id = (int)$_GET['loc_id'];
		$adv_info_tbl->get_db_vars($this->loc_id);
		if(isset($_GET['alt_loc_id'])) {
		  $this->alt_loc_id = (int)$_GET['alt_loc_id'];
		...
		$this->category = (isset($_SESSION['previous_cat_id']) ? $_SESSION['previous_cat_id'] : '');
	...
	private function bread_crumbs() {
		global $cats_tbl, $url_nms_tbl, $zip_cds_tbl, $cities_tbl, $ste_cty_cat_tbl, $api_load;	
			
		// load zip code data
		$zip_cds_tbl->search($this->zip);
			
		// load city data
		$_GET['city'] = $zip_cds_tbl->city_id;
		$cities_tbl->get_db_vars($zip_cds_tbl->city_id);
		
		// added to work with view all page
		if ($this->category > 0) {
			
			// pull category city url name
			$ste_cty_cat_tbl->city_category_search($cities_tbl->id,$cats_tbl->id);
			...
			if($ste_cty_cat_tbl->url_name > 0) {
			  ...
			  if ($api_load->status == 1) {
				...
				$this->bc[] = '<a href="'.SITE_URL.'?page='.urlencode($url_nms_tbl->url_name).'&amp;mode=view-all-results">View All</a>';
			  }
			} else {
				$link_name = 'view-all-results/'. strtolower(preg_replace('/[^a-zA-Z0-9]/', '-', $cities_tbl->city.' '.$cities_tbl->state)).'/'.$cities_tbl->id.'/';
				$link_text = 'View All Deals in '.$cities_tbl->city.', '.$cities_tbl->state;
			}
			...
			// added 12/11/2009 to write urls for api system
			if ($api_load->status != 1) {
			  $this->bc[] = '<a href="'.SITE_URL.'view-all-results/'. strtolower(preg_replace('/[^a-zA-Z0-9]/', '-', $cities_tbl->city.' '.$cities_tbl->state)).'/'.$cities_tbl->id.'/">View All Deals in '.$cities_tbl->city.', '.$cities_tbl->state.'</a>'; 
			}
		...
		if($cities_tbl->url_name > 0) {
			...
			$link_name = 'sections/results.deal?city='.$cities_tbl->id;
		}
	...
	function bread_crumbs_parent($parent_id) {
		global $cats_tbl, $url_nms_tbl, $ste_cty_cat_tbl, $cities_tbl, $api_load;
	...
	public function build_certificate_form() {
		global $adv_info_tbl, $adv_info_tbl;
		  ...
		  if (!empty($cert_req_arr)) {
			  ...
			  $location_info_form = draw_cert_form($this->build_value_drop_down(),$adv_info_tbl->id,$this->default_requirement,$this->build_quantity_drop_down(),'review_'.$adv_info_tbl->id);
	...
	public function print_hours_of_operation() {
	    ...
	    $hour_opp = '<b>Hours of Operation:</b><br/>';
	...
	public function assign_image() {
	  ...
	  if(!empty($adv_info_tbl->image)) {
		  ...
		  $advert_image .= OVERRIDE_SITE_URL.'includes/resize_image.deal?image='.urlencode($location_image).'&amp;new_width='.ADVERTISER_PAGE_IMAGE_WIDTH.'&amp;new_height='.ADVERTISER_PAGE_IMAGE_HEIGHT;
		  $advert_image .= '" alt="'. htmlentities($adv_info_tbl->company_name) .'" />';
	  } else {
		  $advert_image = '<img border="0" src="';
		  $advert_image .= OVERRIDE_SITE_URL.'includes/resize_image.deal?image=&amp;new_width='.ADVERTISER_PAGE_IMAGE_WIDTH.'&amp;new_height='.ADVERTISER_PAGE_IMAGE_HEIGHT;
		  $advert_image .= '" alt="'. htmlentities($adv_info_tbl->company_name) .'" />';
	  }
	...
	public function print_address() {
	  ...
	  if($this->alt_loc_id != '') {
		...
		$address_opp .= '<br/>'.$adv_alt_loc_tbl->phone_number.'</span>';
	  } else {
		$address_opp = '<span class="location_info">';
		if ($adv_info_tbl->hide_address != 1) {
			...
			<a target="_blank" href="http://maps.google.com/maps?f=d&amp;source=s_d&amp;saddr=&amp;daddr='.str_replace(" ","+",$adv_info_tbl->address_1).',+'.str_replace(" ","+",$adv_info_tbl->city).',+'.$adv_info_tbl->state.'+'.$adv_info_tbl->zip.'&amp;hl=en&amp;geocode=&amp;mra=ls&amp;sll='.$adv_info_tbl->latitude.','.$adv_info_tbl->longitude.'&amp;sspn=27.146599,63.28125&amp;ie=UTF8&amp;z=16" style="text-decoration: underline; font-weight: bold;">Get Directions</a>'; 
		}
		$address_opp .= ($adv_info_tbl->email_address != '' ? '<br/>Email: <a href="mailto:'.$adv_info_tbl->email_address.'">'.$adv_info_tbl->email_address.'</a>' : '');
		$address_opp .= '<br/>'.$adv_info_tbl->phone_number.'</span>';
	  }
