...
if (isset($_POST['form_submit'])) {
	...
	// added to check for promocode entry
	if(empty($_POST['promo_code']) || $adv_pro_codes_tbl->promo_code_chk($_POST['promo_code']) > 0) {
	  ...
	  if($adv_lvls_tbl->level_renewal_cost > 0) {
		  ...
		  header("Location: ".SITE_SSL_URL."advertiser_admin/advertiser_payment_options.deal");
	  } else {
		...
		if(!empty($_POST['promo_code']) && $adv_pro_codes_tbl->promo_code_chk($_POST['promo_code']) == 0) {
			$page_output .= create_warning_box('The promo code you entered does not appear to be valid.');
		}
	}
}
...
// cycle through all account levels and print form
		...
		foreach($account_levels as $value) {
			...
			$level_desc = str_replace('"','\"',preg_replace('/\>\s+\</', '> <', $value['level_description']));
			
			$level_desc = explode("\n",$level_desc);
			$level_desc = implode(" ",$level_desc);
			
			$requirement_value .= 'curArray['.$value['id'].'] = "'.$level_desc.'";'.LB.LB; 
		...
		foreach($account_levels as $value) {
			$value_drop_down[] .= '<tr><td><input name="advertiser_level_id" class="advertiser_level_id" type="radio" value="'.$value['id'].'" '.($value['id'] == $adv_info_tbl->customer_level ? 'checked' : $value['id'] == 3 ? 'checked' : '' ).' /> '.$value['level_name'].'</td></tr>';
		}
...
$page_output .= '<tr>'.LB;
$page_output .= '<th align="center">Promo Code</th>'.LB;
$page_output .= '</tr>'.LB;
$page_output .= '<tr>'.LB;
$page_output .= '<td><table align="center"><tr><td>Enter a promo code to save on a paid level selection.</td></tr><tr><td align="center"><input name="promo_code" size="10" maxlength="10" value="'.$adv_info_tbl->promo_code.'"></td></tr></table></td>'.LB;
$page_output .= '</tr>'.LB;
...
//$page_output .= '</tr>'.LB;
$page_output .= '<tr>'.LB;
$page_output .= '<th align="center">&nbsp;</th>'.LB;
$page_output .= '</tr>'.LB;
$page_output .= '<tr><td align="center"><input class="submit_btn" name="Submit" type="submit" value="Submit"></td></tr></table>';
$page_output .= '</td>
                    </form>';
}
