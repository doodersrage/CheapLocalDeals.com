...
// query active certificates
		...
		if(count($selected_rows) == 0) {
			$page_output .= '<span class="cert-error">No reviews awaiting approval were found.</span>';
		} else {
			// print list of active certificates
			$page_output .= '<center><strong>Reviews Awaiting Approval</strong></center>';
			$page_output .= '<table align="center" class="frn_box">';
			$page_output .= '<tr><th class="shop_header">Row</th><th class="shop_header">Customer Name</th><th class="shop_header">Rating</th><th class="shop_header">Review</th><th class="shop_header">Added</th><th class="shop_header">Approve</th><th>Disapprove</th></tr>';
			$sel_row = 0;
			...
			foreach($selected_rows as $cur_row) {
				...
				<td class="frn_conbox"><a href="javascript:void(0)" onclick="approve_review('.$adv_rvws_tbl->id.')" >Approve</a></td>
				<td class="frn_conbox"><a href="javascript:void(0)" onclick="disapprove_review('.$adv_rvws_tbl->id.')" >Disapprove</a></td>
				</tr>';				
			}
			$page_output .= '</table>';
		}
