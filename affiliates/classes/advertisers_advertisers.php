...
// class used for advertisers to advertiser report
class advertisers_advertisers_report {

	// display advertisers affiliates listing
	function advertisers_tracking_listing($message = '') {
		...
		$page_view .= $this->advertisers_tracking_listing_content();
		$page_view .= close_table_form();
		
	return $page_view;
	}
	
	function advertisers_tracking_listing_content() {
			global $dbh, $pgs_tbl;
		...
		// draw table header
		$searchbox_head = array('<form action="" method="post" name="search_form">Search: <input name="search_box" type="text" size="35"><input name="submit" type="submit" value="Submit">');
		...
		if (empty($_POST['search_box'])) {
			...
			for($i = 0;$i <= $page_count;$i++) {
				$pages_array[] = '<a href="?sect=salesrepreports&mode=signedup&page_val='.($i*$page_limiter).'">'.($_GET['page_val'] == $i*$page_limiter ? '<font color="red">' : '').($i+1).($_GET['page_val'] == $i*$page_limiter ? '</font>' : '').'</a>';
			}
			
			$pages_links = implode(', ',$pages_array);
			
			$pages_listing .= table_listing_span_form_field('<center>Total Advertisers: '.$row_count.'<br>Pages:<br>'.$pages_links.'</center>');
		}
		...
		$sql_query .= "
				...
				'%".strtolower(str_replace("'","''",$_POST['search_box']))."%' OR username like '%".str_replace("'","''",$_POST['search_box'])."%' OR last_name like '%".str_replace("'","''",$_POST['search_box'])."%') ";
		}
		if (!empty($_POST['start_date']) && empty($_POST['search_box'])) {
		$sql_query .= "WHERE 
		...
		foreach ($rows as $pages) {
			...
			$row_array = array(
								'<a href="'.SITE_AFFILIATE_SSL_URL.'?sect=salesrepreports&mode=advertdata&pid='.$pages['affiliate_code'].'">'.$pages['company_name'].'</a>',
								$pages['affiliate_code'],
								$pages['link_affiliate_code'],
								);
		...
		$pages_listing .= draw_table_contect(array('<center>Total Advertisers: '.$row_count.'<br>Pages:<br>'.$pages_links.'</center>'),$table_boxes_cnt,'center');
	...
	function affiliates_advertisers_listing($message = '') {
		$page_view = open_table_listing_form('Affiliate Advertisers Listing','view_affiliates_advertisers','','post',$message);
	...
	function affiliates_advertisers_listing_content() {
		...
		foreach ($rows as $pages) {
			...
			$row_array = array(
								...
								'<a href="'.SITE_AFFILIATE_SSL_URL.'?sect=salesrepreports&mode=advertiserlistreport&pid='.$_GET['pid'].'&advertid='.$pages['id'].'">'.$pages['company_name'].'</a>',
		...
		$footer_array = array('<a href="'.SITE_AFFILIATE_SSL_URL.'?sect=affiliatesreports&mode=advertiserlist&pid='.$_GET['pid'].'&action=download_xls'.(!empty($_POST['start_date']) ? '&start_date='.urlencode($_POST['start_date']).'&end_date='.urlencode($_POST['end_date']) : '' ).'">Save as Excel Spreadsheet</a>');
		$pages_listing .= draw_table_header($footer_array,$table_boxes_cnt,'center');
