...
if (!class_exists('forget_password_pg')) {
	require(CLASSES_DIR.'pages/forget_password.php');
	$forget_password_pg = new forget_password_pg;
}
	...
	if ($_POST['submit'] === 'Generate Password') {
		...
		if (!empty($_POST['email'])) {
			...
			if ($customer_info_table->user_forget_password_check() > 0) {				
				...
				$page_output .= $forget_password_pg->draw_forget_password_form();					
			}
